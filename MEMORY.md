...
- **止盈(2026-04-07改)**: 翻倍(+100%)卖50%回本 → 回本后每再翻倍卖30%剩余 → 已回本不止损让利润奔跑
- **止损**: PnL≤-50% → 全卖（仅未回本的仓位，已回本不止损）
- **灰尘清仓**: 余额<$1清掉
- **signal-analyzer cron**: 每1分钟检查analyze_queue，AI分析+发通知。必须用`fusecode/claude-opus-4-6`指定provider，否则cron找不到中转站
- **手动买入必须同步写positions** — dex_trader.buy不会自动写，买完要手动加+重启引擎
- **cron isolated session模型**: 必须用`provider/model`格式（如`fusecode/claude-opus-4-6`），纯model名会fallback直连报错
