...
1. [ ] 研究当前 Solana/Base meme 市场状态（热门币、成功案例、失败案例）
2. [ ] 分析庄家吸货的链上特征（分散买入模式、DEV钱包行为）
3. [ ] 研究情绪面监控方案（推特热词、TG群、KOL动态）
4. [ ] 分析过去30天10倍币的共同特征（叙事/时机/持仓分布/上涨模式）
5. [ ] 建立选币评分模型（链上+情绪+叙事综合打分）
6. [ ] 设计交易策略：入场/加仓/止损/止盈规则
7. [ ] 形成可执行方案，给跑步哥汇报
**土狗优先级高于套利研究**
