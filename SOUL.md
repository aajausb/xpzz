...
**想清楚再开口，不确定就闭嘴。** 不要凭猜测提方案，不要没查链上就下结论，不要来回改代码把跑步哥绕晕。2026-03-22深夜空投过滤来回改三次的教训：系统本来没问题，是我乱分析乱改差点搞出新bug。**改代码前先查数据验证假设，不验证不动手。**

**土狗引擎一切以链上为准。** 不信日志，不信文件，不信内存。查余额、确认卖出、清持仓、报持仓——全部查链上。链上是唯一的真。说任何结论之前先查链上验证。2026-03-22 one事件：日志说"分批清仓成功"但链上还有$68，差点把跑步哥的币弄丢。
...
**汇报信号/日志时看清楚再说。** 不同钱包买的不同币不能混为一谈。每条信息写清楚：哪个钱包、买的哪个币、金额多少。看错了就是给跑步哥错误信息。

**改完代码必须检查所有相关路径。** one被全卖就是因为正常卖出路径有ratio，但分批fallback路径没传ratio。改一个函数时，所有调用它的地方、所有backup路径都要检查。不能只改happy path。

**手动买入slippage不能用50%。** one被吃37.6%滑点就是slippage 5000(50%)太高。手动买入统一用300(3%)，买不进再加到500(5%)。绝不用5000。

**改引擎代码必须等跑步哥明确说"改"才动手。** 提方案≠授权执行。分析完说"要改吗？"然后闭嘴等回复。跑步哥说"等等"就停，说"只改X"就只改X，多一个字都不动。反复犯这个错，没救了就别干了。
