...
const SKIP_SYMBOLS = new Set(['USDT','USDC','BUSD','DAI','TUSD','FDUSD','USD1','JUPUSD','PYUSD','GUSD','USDP','SUSD','LUSD','CUSD','DOLA','FRAX','USDD','CRVUSD','MKUSD','EURC','USDY','USDM','USDe','WBNB','WETH','WBTC','CBBTC','CBETH','STETH','RETH','BNB','ETH','BTC','SOL','WSOL','MSOL','JITOSOL','BNSOL']);
let auditCache = {};
...
// SOL RPC: 官方→PublicNode（QuickNode已停）
const SOL_RPCS = [
...
async function checkEvmSellOrTransfer(chain, smWallet, tokenAddress) {
  try {
    ...
    if (!lastIn || lastOut.blockNumber > lastIn.blockNumber || 
        (lastOut.blockNumber === lastIn.blockNumber && (lastOut.index || 0) > (lastIn.index || 0))) {
...
const bscProvider = new ethers.JsonRpcProvider('https://bsc-dataseed1.binance.org', 56, { staticNetwork: true });
const baseProvider = new ethers.JsonRpcProvider('https://blue-fluent-vineyard.base-mainnet.quiknode.pro/b732585029ad7cdbd7fcbe4c4729884a3b211d5c/', 8453, { staticNetwork: true });
...
const saveJSON = (file, data) => {
  ...
  if (file.includes('positions') && typeof data === 'object') {
    ...
    if (keys.length !== jsonKeys.length) {
      log('ERROR', `⚠️ positions序列化丢失! 内存${keys.length}个 序列化${jsonKeys.length}个 丢失keys: ${keys.filter(k=>!jsonKeys.includes(k)).join(',')}`);
    }
  }
...
async function checkSolTransferTarget(tokenAddr, smWallet) {
  try {
    ...
    for (const sig of sigs.result) {
      ...
      // 查token transfer的目标地址 — 只匹配SPL Token转账且mint匹配我们跟踪的token
      ...
      for (const inner of innerInstructions) {
        for (const inst of (inner.instructions || [])) {
          const prog = inst.programId || inst.program || '';
          const isTokenProg = TOKEN_PROGRAMS.has(prog) || prog === 'spl-token';
          if (!isTokenProg) continue;
          const p = inst.parsed;
          if (!p) continue;
          // transferChecked 带 mint 字段，直接比对
          if (p.type === 'transferChecked' && p.info?.mint === tokenAddr) {
            return { type: 'transfer', to: p.info.destination };
          }
          // transfer 没有 mint，需要查 token account 的 mint — 跳过，不够可靠
        }
      }
    }
    return { type: 'unknown' };
...
const SOL_WS_OFFICIAL = 'wss://morning-dry-market.solana-mainnet.quiknode.pro/6664c189556346b5503ea032fb269e81291957ab/'; // 公共WS（有订阅限制）
...
async function parseSolSignature(walletAddr, signature) {
  ...
  try {
    // 用QuickNode getTransaction解析swap（不依赖Helius），带重试防truncated JSON
    let txData;
    for (let _retry = 0; _retry < 2; _retry++) {
      try {
        txData = await rpcPost(getSolRpc(), 'getTransaction', [signature, { encoding: 'jsonParsed', maxSupportedTransactionVersion: 0 }]);
        break;
      } catch (retryErr) {
        if (_retry === 0 && retryErr.message?.includes('JSON')) {
    ...
    for (const p of post) {
      ...
      if (diff > 0) {
        // 买入: 余额增加
        // 计算SM投入金额：直接查链上持有余额×OKX价格
        const smBuyAmountUsd = 0; // 达标审计时再查SM持有价值
    ...
    // 检查pre里有但post里没有的token（全卖+关ATA的情况）
    for (const preItem of pre) {
      if (preItem.owner !== walletAddr) continue;
      if (preItem.mint === WSOL) continue;
      const preAmt = parseFloat(preItem?.uiTokenAmount?.uiAmount || 0);
      if (preAmt <= 0) continue;
      // 看post里有没有同mint+owner
      const inPost = post.some(p => p.owner === walletAddr && p.mint === preItem.mint);
      if (!inPost && positions[preItem.mint]) {
        // 全卖+关ATA：pre有余额，post里消失了
        log('INFO', `📉 [SOL] 全卖信号! 钱包#${rank} ${walletAddr.slice(0,10)}... 清仓 ${preItem.mint} (ATA已关闭，待巡检确认)`);
      }
    }
  } catch(e) {
...
const EVM_WS_ENDPOINTS = {
  bsc: ["wss://bsc.publicnode.com", "wss://bsc.publicnode.com"],
  base: ["wss://blue-fluent-vineyard.base-mainnet.quiknode.pro/b732585029ad7cdbd7fcbe4c4729884a3b211d5c/", "wss://base.publicnode.com"],
};
    ...
    ws.on("message", async (data) => {
      try {
        ...
        if (msg.params?.result) {
          ...
          if (!logEntry.topics || logEntry.topics.length < 3) return;
          ...
          if (walletSet.has(toAddr) && !walletSet.has(fromAddr) && !nativeTokens.has(tokenAddr)) {
            ...
            const rank = wallet?.rank || 999;
                        const smBuyAmountUsd = 0; // 达标审计时再查
  ...
  while (true) {
    for (const chainKey of ['bsc', 'base']) {
      ...
      try {
        ...
        if (!_evmPollLastBlock[chainKey]) _evmPollLastBlock[chainKey] = currentBlock - 1;
        ...
        if (hunters.length === 0) { _evmPollLastBlock[chainKey] = currentBlock; continue; }
...
async function verifyEvmSell(chainKey, txHash, walletAddr, tokenAddr) {
  ...
  } catch(e) { log("WARN", `verifyEvmSell异常: ${e.message?.slice(0,60)}`); }
}
...
// EVM地址小写比较，SOL地址原样比较
const SKIP_TOKENS_EVM = new Set([...SKIP_TOKENS].filter(a => a.startsWith('0x')).map(a => a.toLowerCase()));
const SKIP_TOKENS_SOL = new Set([...SKIP_TOKENS].filter(a => !a.startsWith('0x')));
...
async function handleSignal(signal) {
  ...
  // 稳定币/大币地址直接跳过（不等审计）
  const isSkip = chain === 'solana' ? SKIP_TOKENS_SOL.has(token) : SKIP_TOKENS_EVM.has(token.toLowerCase());
  if (isSkip) return;
  ...
  if (!alreadyCounted) {
    ...
    if (existing) {
      ...
      existing.timestamp = signal.timestamp; // 刷新时间戳，防过期
      if (signal.walletStatus === 'hunter' && existing.walletStatus !== 'hunter') existing.walletStatus = signal.walletStatus;
    }
  }
  ...
  if (symbol === '?' && chain === 'solana') {
    try {
      ...
      // Token-2022: metadata在extensions里
      const exts = parsed?.info?.extensions;
      const tokenMeta = exts?.find(e => e.extension === 'tokenMetadata');
      if (tokenMeta?.state?.symbol) symbol = tokenMeta.state.symbol;
      else if (tokenMeta?.state?.name) symbol = tokenMeta.state.name.slice(0, 20);
      // SPL Token: 查Metaplex metadata PDA
      if (symbol === '?') {
        const METADATA_PID = 'metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s';
        const { PublicKey: PK } = require('@solana/web3.js');
        const [metaPda] = PK.findProgramAddressSync(
          [Buffer.from('metadata'), new PK(METADATA_PID).toBuffer(), new PK(token).toBuffer()],
          new PK(METADATA_PID)
        );
        const metaAcct = await rpcPost(getSolRpc(), 'getAccountInfo', [metaPda.toBase58(), { encoding: 'base64' }]);
        const buf = metaAcct.result?.value?.data?.[0] ? Buffer.from(metaAcct.result.value.data[0], 'base64') : null;
        if (buf && buf.length > 73) {
          const nameLen = buf.readUInt32LE(65);
          if (nameLen > 0 && nameLen < 100) {
            const symOff = 69 + nameLen;
            const symLen = buf.readUInt32LE(symOff);
            if (symLen > 0 && symLen < 50) {
              symbol = buf.slice(symOff + 4, symOff + 4 + symLen).toString('utf8').replace(/\0/g, '').trim();
            }
          }
        }
      }
  ...
  // 审计时查到symbol后回写到pendingSignals（消灭?）
  if (symbol !== '?' && pendingSignals[token]) {
    for (const s of pendingSignals[token]) { if (s.symbol === '?') s.symbol = symbol; }
  }
  ...
  for (const s of (pendingSignals[token] || [])) {
    if (s.walletStatus === 'hunter' || s.walletStatus === 'scout') {
      smWalletAmounts[s.wallet] = s.smBuyAmountUsd || 0;
    }
  }
  ...
  try {
    ...
    if (price > 0) {
      ...
      for (const w of holdingWallets) {
        filteredAmounts[w] = smWalletAmounts[w] || 0;
      }
...
async function auditToken(chain, tokenAddress) {
  // 检查缓存（safe结果24小时过期，buy_failed也24小时过期，其他unsafe永久缓存）
  const cached = auditCache[tokenAddress];
  if (cached) {
    if (!cached.safe) {
      // buy_failed/buy_reverted 24小时后可重试（可能是临时RPC问题）
      if ((cached.reason === 'buy_failed_3x' || cached.reason === 'buy_reverted_3x') && cached._time && Date.now() - cached._time > 24 * 3600 * 1000) {
        // 过期，重新审计
      } else {
        return cached;
      }
...
async function _executeBuyInner(chain, tokenAddress, symbol, confirmCount, confirmWallets, smWalletAmounts = {}, watchCount = 0, _retryDepth = 0) {
  let smTotal = 0; // SM持有总价值（函数顶层定义，后续查链上赋值）
  // 二次检查持仓上限（防并发穿透）
  ...
  for (let pi = 0; pi < 3; pi++) {
    try { price = await getNativePrice(chain); } catch {}
    if (price && price > 0) break;
    if (pi < 2) await sleep(2000);
  }
  try {
    ...
    // SM持有价值：查链上余额×DexScreener价格
    smTotal = 0;
    try {
      const confirmWalletAddrs = Object.keys(smWalletAmounts);
      const dexData2 = await httpGet(`https://api.dexscreener.com/latest/dex/tokens/${tokenAddress}`).catch(() => null);
      let tokenPrice = parseFloat(dexData2?.pairs?.[0]?.priceUsd || 0);
      // DexScreener没价格时用OKX quote fallback
      if (tokenPrice === 0) {
        try {
          const chainId = chain === 'solana' ? '501' : chain === 'bsc' ? '56' : '8453';
          const native = chain === 'solana' ? 'So11111111111111111111111111111111111111112' : '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE';
          const amt = chain === 'solana' ? '100000000' : '100000000000000000'; // 0.1 SOL or 0.1 ETH/BNB
          const okxPath = `/api/v6/dex/aggregator/quote?chainIndex=${chainId}&fromTokenAddress=${native}&toTokenAddress=${tokenAddress}&amount=${amt}`;
          const okxTs = new Date().toISOString();
          const okxSign = require('crypto').createHmac('sha256', process.env.OKX_SECRET_KEY || '').update(okxTs + 'GET' + okxPath).digest('base64');
          const okxQ = await httpGet(`https://web3.okx.com${okxPath}`, {
            'OK-ACCESS-KEY': process.env.OKX_API_KEY || '',
            'OK-ACCESS-SIGN': okxSign,
            'OK-ACCESS-TIMESTAMP': okxTs,
            'OK-ACCESS-PASSPHRASE': process.env.OKX_PASSPHRASE || '',
            'OK-ACCESS-PROJECT': '',
          }).catch(() => null);
          const toAmt = parseFloat(okxQ?.data?.[0]?.toTokenAmount || 0);
          const toDecimal = parseInt(okxQ?.data?.[0]?.toToken?.decimal || 0);
          if (toAmt > 0 && toDecimal > 0) {
            const nativeD = await httpGet(`https://api.dexscreener.com/latest/dex/tokens/${NATIVE_TOKENS[chain]}`).catch(() => null);
            const nativePrice = parseFloat(nativeD?.pairs?.[0]?.priceUsd || 0);
            const inputUsd = chain === 'solana' ? 0.1 * nativePrice : 0.1 * nativePrice;
            const tokensOut = toAmt / (10 ** toDecimal);
            if (tokensOut > 0) tokenPrice = inputUsd / tokensOut;
            if (tokenPrice > 0) log('INFO', `💰 ${symbol} OKX fallback价格: $${tokenPrice.toFixed(8)}`);
          }
        } catch {}
      }
      if (tokenPrice > 0 && confirmWalletAddrs.length > 0) {
        if (chain === 'solana') {
          for (const w of confirmWalletAddrs) {
            const bal = await getSolTokenBalance(w, tokenAddress);
            if (bal > 0) smTotal += bal * tokenPrice;
          }
        } else {
          const { ethers } = require('ethers');
          const provider = chain === 'bsc' ? bscProvider : baseProvider;
          ...
          for (const w of confirmWalletAddrs) {
            const rawBal = await erc20.balanceOf(w).catch(() => 0n);
            const bal = parseFloat(ethers.formatUnits(rawBal, dec));
            if (bal > 0) smTotal += bal * tokenPrice;
          }
        }
      }
    } catch(e) { log('WARN', `SM持有查询失败: ${e.message?.slice(0,40)}`); }
    log('INFO', `💰 ${symbol} SM持有总价值$${Math.round(smTotal)} (${Object.keys(smWalletAmounts).length}个SM)`);
    if (smTotal < 500) {
    ...
    // 统一$300
    size = 300;
    log('INFO', `📊 ${symbol} 仓位$${size} (猎手${confirmCount}哨兵${watchCount} SM累计$${Math.round(smTotal)})`);
  ...
  try {
    ...
    if (result.success) {
            ...
            if (buyAmount === 0) {
              try {
                ...
                if (qRes?.data?.[0]?.toTokenAmount) {
                  buyPrice = parseFloat(qRes.data[0].fromToken?.tokenUnitPrice || qRes.data[0].fromTokenUsdPrice || 0);
                }
              } catch {}
              if (buyPrice > 0) {
      ...
      const smTotalAtBuy = smTotal; // 用链上查到的SM持有价值
      // 实际成交价：优先用OKX聚合器返回的报价，最准确
      if (buyAmount > 0) {
        const rr = result.routerResult;
        if (rr && rr.fromTokenAmount && rr.fromToken?.tokenUnitPrice) {
          // OKX报价：fromTokenAmount是花的原生币raw量，fromToken.tokenUnitPrice是原生币USD价
          const fromDec = parseInt(rr.fromToken.decimal || (chain === 'solana' ? 9 : 18));
          const nativeSpent = parseFloat(rr.fromTokenAmount) / Math.pow(10, fromDec);
          const nativePrice = parseFloat(rr.fromToken.tokenUnitPrice);
          const realCostUsd = nativeSpent * nativePrice;
          buyPrice = realCostUsd / buyAmount;
          log('INFO', `📊 ${symbol} OKX成交价: $${buyPrice.toFixed(8)} (花费${nativeSpent.toFixed(4)} ${chain==='solana'?'SOL':'ETH'} = $${realCostUsd.toFixed(2)})`);
        } else {
          // fallback: 花费/token数
          buyPrice = size / buyAmount;
          log('INFO', `📊 ${symbol} 估算成交价: $${buyPrice.toFixed(8)} (无OKX数据，用$${size}/数量)`);
        }
      }
      ...
      // EVM链买入后立刻approve max（卖出时不用等）
      if (chain !== 'solana') {
        const _ta2 = tokenAddress, _sym3 = symbol;
        setTimeout(async () => {
          try {
            await require('./dex_trader.js').preApprove(chain, _ta2);
          } catch(e) { log('WARN', `${_sym3} 预approve失败: ${e.message?.slice(0,40)}`); }
        }, 5000);
      }
      ...
      if (symbol === '?') {
        const _token = tokenAddress, _chain = chain;
        setTimeout(async () => {
          try {
      ...
      if (result.txHash && (result.error || '').includes('确认超时')) {
        ...
        if (actualAmount > 0) {
          ...
          const allSMForToken2 = (pendingSignals[tokenAddress] || []).map(s => s.wallet).filter(Boolean);
          const uniqueAllSM2 = [...new Set([...confirmWallets, ...allSMForToken2])];
          positions[tokenAddress] = {
            chain, token: tokenAddress, symbol, buyPrice, buyAmount: actualAmount,
            buyAmountRaw: actualAmount, buyCost: size, buyTime: Date.now(),
            confirmCount, confirmWallets, allSMWallets: uniqueAllSM2,
          };
          boughtTokens.add(tokenAddress);
          saveJSON(POSITIONS_FILE, positions);
          saveJSON(BOUGHT_TOKENS_FILE, [...boughtTokens]);
  ...
  if (!txSucceeded && !positions[tokenAddress]) {
    ...
    if (smStillHolding > 0) {
      ...
      // 递归重试（SM还在就继续买，最多5轮）
      if (_retryDepth >= 5) {
        log('WARN', `⚠️ ${symbol} SM仍在但已重试5轮，放弃`);
        await notifyTelegram(`❌ ${symbol}(${chain}) SM仍在但重试5轮失败，放弃`);
      } else {
        return await _executeBuyInner(chain, tokenAddress, symbol, confirmCount, confirmWallets, smWalletAmounts, watchCount, _retryDepth + 1);
      }
    }
...
async function _executeSellInner(tokenAddress, pos, reason, ratio) {
  // 灰尘检查：链上余额价值<$1直接清仓，不尝试卖（防反复unsellable循环）
  try {
    const trader = require('./dex_trader.js');
    let dustValue = Infinity;
    if (pos.chain === 'solana') {
      let rawBal = 0n;
      for (const pid of ['TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA', 'TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb']) {
        try {
          const balData = await rpcPost(getSolRpc(), 'getTokenAccountsByOwner', [
            trader.getWalletAddress('solana'), { mint: tokenAddress }, { encoding: 'jsonParsed', programId: pid }
          ]);
          for (const a of (balData.result?.value || [])) rawBal += BigInt(a.account?.data?.parsed?.info?.tokenAmount?.amount || '0');
        } catch {}
      }
      const uiAmount = parseFloat(pos.buyAmount > 0 ? (Number(rawBal) / (pos.buyAmountRaw > 0 ? pos.buyAmountRaw / pos.buyAmount : 1e6)) : 0);
      dustValue = uiAmount * (pos.currentPrice || pos.buyPrice || 0);
    } else {
      const { ethers } = require('ethers');
      const provider = pos.chain === 'bsc' ? bscProvider : baseProvider;
      const erc20 = new ethers.Contract(tokenAddress, ['function balanceOf(address) view returns (uint256)', 'function decimals() view returns (uint8)'], provider);
      const dec = await erc20.decimals().catch(() => 18);
      const bal = await erc20.balanceOf(trader.getWalletAddress('evm')).catch(() => 0n);
      const uiAmount = parseFloat(ethers.formatUnits(bal, dec));
      dustValue = uiAmount * (pos.currentPrice || pos.buyPrice || 0);
    }
    if (dustValue < 1) {
      log('INFO', `🧹 ${pos.symbol}(${pos.chain}) 链上余额价值$${dustValue.toFixed(4)}<$1，灰尘清仓`);
      const finalPnl = ((pos.sellRevenue || 0) - (pos.buyCost || 0)).toFixed(2);
      await notifyTelegram(`🧹 ${pos.symbol}(${pos.chain}) 灰尘清仓 | 成本$${pos.buyCost||0} 回收$${(pos.sellRevenue||0).toFixed(2)} 盈亏$${finalPnl}`);
  ...
  } catch(e) { log('WARN', `灰尘检查异常 ${pos.symbol}: ${e.message?.slice(0,50)}`); }
  ...
  for (let attempt = 1; attempt <= SELL_MAX_RETRIES; attempt++) {
    try {
      ...
      let result;
      let _preNativeBal = 0; // 卖出前原生币余额（用于计算真实回收）
      if (ratio < 0.99) {
        // 查链上余额算部分数量
        let partialAmount;
        try {
        ...
        // 链上余额=0时不估算，直接跳过（防止精度错误导致异常卖出）
        if (!partialAmount || partialAmount === '0') {
          log('WARN', `链上余额查询失败或为0，跳过部分卖出（不用估算值，防精度错误）`);
          return;
        }
        // 记录卖出前原生币余额（用于计算真实回收）
        _preNativeBal = 0;
        try {
          if (pos.chain === 'solana') {
            const { Connection: CS, PublicKey: PS } = require('@solana/web3.js');
            const cs = new CS(getSolRpc());
            _preNativeBal = (await cs.getBalance(new PS(require('./dex_trader.js').getWalletAddress('solana')), 'confirmed')) / 1e9;
          } else {
            const provS = pos.chain === 'bsc' ? bscProvider : baseProvider;
            const balS = await provS.getBalance(require('./dex_trader.js').getWalletAddress('evm'));
            _preNativeBal = parseFloat(require('ethers').formatEther(balS));
          }
        } catch {}
      ...
      if (result.success) {
        ...
        try {
          ...
          if (pos.chain === 'solana') {
            ...
            const erc2 = new eth2.Contract(tokenAddress, ['function balanceOf(address) view returns (uint256)', 'function decimals() view returns (uint8)'], prov2);
            const b2 = await erc2.balanceOf(trader2.getWalletAddress('evm'));
            let dec2 = 18; try { dec2 = await erc2.decimals(); } catch {}
            postBal = parseFloat(eth2.formatUnits(b2, dec2));
          }
            ...
            try {
              ...
              if (splitBal && splitBal !== '0' && Number(splitBal) > 0) {
                ...
                if (pos.chain === 'solana') {
                  ...
                  const erc4 = new eth4.Contract(tokenAddress, ['function balanceOf(address) view returns (uint256)', 'function decimals() view returns (uint8)'], prov4);
                  const fb = await erc4.balanceOf(trader3.getWalletAddress('evm'));
                  let dec4 = 18; try { dec4 = await erc4.decimals(); } catch {}
                  finalBal = parseFloat(eth4.formatUnits(fb, dec4));
                }
        ...
        // 计算真实卖出回收（原生币余额差）
        try {
          let _postNativeBal = 0;
          if (pos.chain === 'solana') {
            const { Connection: CP, PublicKey: PP } = require('@solana/web3.js');
            const cp = new CP(getSolRpc());
            _postNativeBal = (await cp.getBalance(new PP(require('./dex_trader.js').getWalletAddress('solana')), 'confirmed')) / 1e9;
          } else {
            const provP = pos.chain === 'bsc' ? bscProvider : baseProvider;
            const balP = await provP.getBalance(require('./dex_trader.js').getWalletAddress('evm'));
            _postNativeBal = parseFloat(require('ethers').formatEther(balP));
          }
          if (_preNativeBal > 0 && _postNativeBal > _preNativeBal) {
            const nativeDiff = _postNativeBal - _preNativeBal;
            const nativePrice = await getNativePrice(pos.chain);
            const revenue = nativeDiff * nativePrice;
            if (revenue > 0) {
              pos.sellRevenue = (pos.sellRevenue || 0) + revenue;
              log('INFO', `💵 卖出真实回收: $${revenue.toFixed(2)} (${nativeDiff.toFixed(6)} ${pos.chain==='solana'?'SOL':'ETH'}) 累计$${pos.sellRevenue.toFixed(2)}`);
            }
          } else {
            // fallback: 用当前价×数量估算
            ...
            if (revenue > 0) {
              pos.sellRevenue = (pos.sellRevenue || 0) + revenue;
              log('INFO', `💵 卖出估算回收: $${revenue.toFixed(2)} (余额差查询失败，用现价估算) 累计$${pos.sellRevenue.toFixed(2)}`);
            }
          }
        } catch {}
        if (ratio >= 0.99) {
        ...
        let realBal = 0;
        try {
          await sleep(5000); // 等链上确认
          if (pos.chain === 'solana') {
            // 查3次，取最大值，防止RPC偶尔返回0
            for (let qi = 0; qi < 3; qi++) {
              const b = await getSolTokenBalance(require('./dex_trader.js').getWalletAddress('solana'), tokenAddress);
              if (b > realBal) realBal = b;
              if (realBal > 0) break;
              if (qi < 2) await sleep(2000);
            }
          } else {
            const erc = new ethers.Contract(tokenAddress, ['function balanceOf(address) view returns (uint256)', 'function decimals() view returns (uint8)'], pos.chain === 'bsc' ? bscProvider : baseProvider);
            const rawBal = await erc.balanceOf(require('./dex_trader.js').getWalletAddress('evm'));
            const dec = await erc.decimals().catch(() => 18);
            realBal = parseFloat(ethers.formatUnits(rawBal, dec));
          }
        } catch {}
        if (realBal > 0) {
...
async function _trySplitSell(pos, tokenAddress, sellRatio = 1) {
  ...
  try {
    ...
    if (pos.chain === 'solana') {
      ...
      // 记录分批卖出前的原生币余额（用于计算收益）
      let splitPreNativeBalance = 0;
      try {
        const { Connection: C0, PublicKey: P0 } = require('@solana/web3.js');
        const c0 = new C0(getSolRpc());
        const preBal = await c0.getBalance(new P0(trader.getWalletAddress('solana')), 'confirmed');
        splitPreNativeBalance = preBal / 1e9;
      } catch {}
      let ok = 0;
      for (let i = 0; i < 2; i++) {
        try {
          ...
          if (i === 1) {
            try {
              const balData2 = await rpcPost(getSolRpc(), 'getTokenAccountsByOwner', [
                trader.getWalletAddress('solana'), { mint: tokenAddress }, { encoding: 'jsonParsed' }
              ]);
              let remain = 0n;
              for (const a of (balData2.result?.value || [])) remain += BigInt(a.account?.data?.parsed?.info?.tokenAmount?.amount || '0');
              if (remain > 0n) sellAmt = remain.toString();
            } catch {}
          }
          const r = await trader.solanaSell(tokenAddress, sellAmt, 5000);
      ...
      if (ok > 0) {
        ...
        if (positions[tokenAddress]) {
          ...
          // 计算分批卖出收益：用卖后SOL余额 - 卖前SOL余额（splitPreNativeBalance在分批前记录）
          let splitRevenue = 0;
          try {
            if (pos.chain === 'solana') {
              const { Connection: C3, PublicKey: P3 } = require('@solana/web3.js');
              const c3 = new C3(getSolRpc());
              const postBal = await c3.getBalance(new P3(require('./dex_trader.js').getWalletAddress('solana')), 'confirmed');
              const postSol = postBal / 1e9;
              // splitPreNativeBalance在分批卖出前记录
              if (typeof splitPreNativeBalance === 'number' && splitPreNativeBalance > 0) {
                const diffSol = postSol - splitPreNativeBalance;
                if (diffSol > 0) {
                  const solPrice = await getNativePrice('solana');
                  splitRevenue = diffSol * solPrice;
                }
              }
            } else {
              const prov2 = pos.chain === 'bsc' ? bscProvider : baseProvider;
              const postBal = await prov2.getBalance(require('./dex_trader.js').getWalletAddress('evm'));
              const postEth = parseFloat(ethers.formatEther(postBal));
              if (typeof splitPreNativeBalance === 'number' && splitPreNativeBalance > 0) {
                const diffEth = postEth - splitPreNativeBalance;
                if (diffEth > 0) {
                  const nativePrice = await getNativePrice(pos.chain);
                  splitRevenue = diffEth * nativePrice;
                }
              }
            }
          ...
          // 链上确认有残留 → 立刻再尝试卖掉残留
          if (chainConfirm.includes('还剩') || chainConfirm.includes('残留') || chainConfirm.includes('未验证')) {
            log('INFO', `🔄 ${pos.symbol} 残留再尝试卖出...`);
            let retryOk = false;
            try {
              const trader2 = require('./dex_trader.js');
              if (pos.chain === 'solana') {
                const balData3 = await rpcPost(getSolRpc(), 'getTokenAccountsByOwner', [
                  trader2.getWalletAddress('solana'), { mint: tokenAddress }, { encoding: 'jsonParsed' }
                ]);
                let remainRaw = 0n;
                for (const a of (balData3.result?.value || [])) remainRaw += BigInt(a.account?.data?.parsed?.info?.tokenAmount?.amount || '0');
                if (remainRaw > 0n) {
                  const rr = await trader2.solanaSell(tokenAddress, remainRaw.toString(), 5000);
                  if (rr.success) { retryOk = true; log('INFO', `✅ ${pos.symbol} 残留卖出成功`); }
                }
              } else {
                const erc3 = new ethers.Contract(tokenAddress, ['function balanceOf(address) view returns (uint256)'], pos.chain === 'bsc' ? bscProvider : baseProvider);
                const remainBal = await erc3.balanceOf(trader2.getWalletAddress('evm'));
                if (remainBal > 0n) {
                  const rr = await trader2.sell(pos.chain, tokenAddress, remainBal.toString(), 5000);
                  if (rr.success) { retryOk = true; log('INFO', `✅ ${pos.symbol} 残留卖出成功`); }
                }
              }
            } catch(e) { log('WARN', `${pos.symbol} 残留卖出失败: ${e.message?.slice(0,40)}`); }
            ...
            if (retryOk) {
              ...
              try {
                let postNativeBal = 0;
                if (pos.chain === 'solana') {
                  const { Connection: CF, PublicKey: PF } = require('@solana/web3.js');
                  const cf = new CF(getSolRpc());
                  postNativeBal = (await cf.getBalance(new PF(require('./dex_trader.js').getWalletAddress('solana')), 'confirmed')) / 1e9;
                } else {
                  const provF = pos.chain === 'bsc' ? bscProvider : baseProvider;
                  const balF = await provF.getBalance(require('./dex_trader.js').getWalletAddress('evm'));
                  postNativeBal = parseFloat(ethers.formatEther(balF));
                }
                if (typeof splitPreNativeBalance === 'number' && splitPreNativeBalance > 0 && postNativeBal > splitPreNativeBalance) {
                  const nativePrice = await getNativePrice(pos.chain);
                  finalRevenue = (postNativeBal - splitPreNativeBalance) * nativePrice;
                  log('INFO', `💰 ${pos.symbol} 总回收(含残留): ~$${Math.round(finalRevenue)}`);
                }
              } catch {}
              const finalPnl = finalRevenue - (pos.buyCost || 0);
              ...
              pos.sellRevenue = totalRevenue;
              pos._splitNotified = true; // 标记已发通知，防止调用方重复通知
              saveJSON(POSITIONS_FILE, positions);
      ...
      const sellTotal = sellRatio < 0.99 ? (bal * BigInt(Math.round(sellRatio * 100)) / 100n) : bal;
      const half = (sellTotal / 2n).toString();
      // 记录分批卖出前的原生币余额（用于计算收益）
      let splitPreNativeBalance = 0;
      try {
        const preBal = await prov.getBalance(trader.getWalletAddress('evm'));
        splitPreNativeBalance = parseFloat(e2.formatEther(preBal));
      } catch {}
      let ok = 0;
      for (let i = 0; i < 2; i++) {
        try {
          ...
          if (i === 1) {
            try {
              const remainBal = await erc.balanceOf(trader.getWalletAddress('evm'));
              if (remainBal > 0n) sellAmt = remainBal.toString();
            } catch {}
          }
          const r = await trader.sell(pos.chain, tokenAddress, sellAmt, 5000);
          if (r.success) { ok++; log('INFO', `✅ ${pos.symbol} 分批第${i+1}次成功`); }
          else log('WARN', `❌ ${pos.symbol} 分批第${i+1}次: ${r.error}`);
      ...
      if (ok > 0) {
        if (positions[tokenAddress]) {
          ...
          let splitRevenue = 0;
          try {
            const postBal = await prov.getBalance(trader.getWalletAddress('evm'));
            const postEth = parseFloat(e2.formatEther(postBal));
            if (splitPreNativeBalance > 0) {
              const diffEth = postEth - splitPreNativeBalance;
              if (diffEth > 0) {
                const nativePrice = await getNativePrice(pos.chain);
                splitRevenue = diffEth * nativePrice;
          ...
          try {
            await new Promise(r => setTimeout(r, 5000));
            const ercCheck = new e2.Contract(tokenAddress, ['function balanceOf(address) view returns (uint256)'], prov);
            const rbCheck = await ercCheck.balanceOf(trader.getWalletAddress('evm'));
            evmRemain = rbCheck === 0n ? 0 : 1;
          } catch {}
          if (evmRemain > 0) {
            pos.sellRevenue = totalRevenue;
            saveJSON(POSITIONS_FILE, positions);
...
// ============ 持仓管理 ============
// OKX钱包API批量查价格（一次调用拿所有token价格）
async function getOkxWalletPrices() {
  const priceMap = {};
  const crypto2 = require('crypto');
  try {
    const chains = [
      { chainIndex: '501', address: require('./dex_trader.js').getWalletAddress('solana') },
      { chainIndex: '8453', address: require('./dex_trader.js').getWalletAddress('evm') }
    ];
    for (const { chainIndex, address } of chains) {
      const ts = new Date().toISOString();
      const path = `/api/v5/wallet/asset/all-token-balances-by-address?chains=${chainIndex}&address=${address}`;
      const sign = crypto2.createHmac('sha256', OKX_ENV.OKX_SECRET_KEY).update(ts + 'GET' + path).digest('base64');
      const r = await httpGet(`https://web3.okx.com${path}`, {
        'OK-ACCESS-KEY': OKX_ENV.OKX_API_KEY, 'OK-ACCESS-SIGN': sign,
        'OK-ACCESS-TIMESTAMP': ts, 'OK-ACCESS-PASSPHRASE': OKX_ENV.OKX_PASSPHRASE
      });
      for (const t of (r?.data?.[0]?.tokenAssets || r?.data || [])) {
        const addr = t.tokenAddress || t.tokenContractAddress;
        const price = parseFloat(t.tokenPrice || 0);
        if (addr && price > 0) priceMap[addr] = price;
      }
    }
  } catch(e) { log('WARN', `OKX钱包API查价失败: ${e.message?.slice(0,50)}`); }
  return priceMap;
}
...
async function managePositions() {
  while (true) {
    ...
    let okxPrices = {};
    if (posKeys.length > 0) {
      try { okxPrices = await getOkxWalletPrices(); } catch {}
    }
    for (const [tokenAddr, pos] of Object.entries(positions)) {
      try {
        const holdTime = Date.now() - pos.buyTime;
        
        // 查价格：OKX钱包API批量结果优先，OKX quote fallback，DexScreener最后
        try {
          let currentPrice = okxPrices[tokenAddr] || 0;
          // OKX钱包API没有 → OKX v6 quote fallback
          if (currentPrice <= 0) {
          try {
            const chainId = CHAIN_ID[pos.chain];
            const native = NATIVE[pos.chain];
            const dec = pos.chain === 'solana' ? 6 : 18; // pump.fun token默认6位，ERC20默认18
            const oneUnit = (BigInt(10) ** BigInt(dec)).toString();
            const qPath = `/api/v6/dex/aggregator/quote?chainIndex=${chainId}&fromTokenAddress=${tokenAddr}&toTokenAddress=${native}&amount=${oneUnit}&slippagePercent=0.5`;
            const qRes = await (async () => {
              const ts = new Date().toISOString();
              const sign = crypto.createHmac('sha256', OKX_ENV.OKX_SECRET_KEY).update(ts + 'GET' + qPath).digest('base64');
              return httpGet(`https://web3.okx.com${qPath}`, {
                'OK-ACCESS-KEY': OKX_ENV.OKX_API_KEY, 'OK-ACCESS-SIGN': sign,
                'OK-ACCESS-TIMESTAMP': ts, 'OK-ACCESS-PASSPHRASE': OKX_ENV.OKX_PASSPHRASE
              });
            })();
            if (qRes?.data?.[0]?.fromToken?.tokenUnitPrice) {
              currentPrice = parseFloat(qRes.data[0].fromToken.tokenUnitPrice);
            }
          } catch(e) { /* OKX失败，fallback DS */ }
          }
          // DexScreener fallback
          if (currentPrice <= 0) {
            const d = await dexScreenerGet(tokenAddr);
            currentPrice = parseFloat(d?.pairs?.[0]?.priceUsd || 0);
          }
          if (currentPrice > 0) {
            ...
            if (pos.buyAmount > 0 && buyAge > 60000) { // 检查链上余额是否=0（手动卖出/部分卖出全卖了）
              try {
                ...
                if (pos.chain === 'solana') {
                  ...
                  const bal = await erc20.balanceOf(trader.getWalletAddress('evm'));
                  const dec = await erc20.decimals().catch(() => 18);
                  ourBal = parseFloat(ethers.formatUnits(bal, dec));
                }
        ...
        //         const smCheckInterval = 120000; // 120秒查一次SM持仓（省RPC credits）
        ...
        //                   if (bal === -1) continue; // RPC全失败，跳过不误判
        ...
        if (pos.currentPrice > 0 && pos.buyPrice > 0) {
          ...
          if (pnlPct <= -50 && buyAge2 > 300000) {
            {
              ...
              if (positions[tokenAddr] && !positions[tokenAddr]._splitNotified) {
                ...
                const rev = pos.sellRevenue || 0;
                const pnl = rev - (pos.buyCost || 0);
                await notifyTelegram(`🔴 止损 ${pos.symbol}(${pos.chain}) ${pnlPct.toFixed(0)}%\n💰 成本$${pos.buyCost||0} 回收$${Math.round(rev)} 盈亏$${Math.round(pnl)}`);
                delete positions[tokenAddr];
                delete sellTracker[tokenAddr];
                delete transferTracker[tokenAddr];
                boughtTokens.delete(tokenAddr);
          ...
          const buyAge3 = Date.now() - (pos.buyTime || 0);
          if (buyAge3 > 300000 && absValue < 1 && pos.buyCost > 1 && absValue > 0) {
            log('INFO', `🧹 ${pos.symbol}(${pos.chain}) 灰尘清仓 价值$${absValue.toFixed(4)}`);
            const rev = pos.sellRevenue || 0;
            const pnl = rev - (pos.buyCost || 0);
            recordTradeHistory(tokenAddr, pos);
            delete positions[tokenAddr];
            ...
            saveJSON(BOUGHT_TOKENS_FILE, [...boughtTokens]);
            saveJSON(POSITIONS_FILE, positions);
            await notifyTelegram(`🧹 灰尘清仓 ${pos.symbol}(${pos.chain})\n💰 成本$${pos.buyCost||0} 回收$${Math.round(rev)} 盈亏$${Math.round(pnl)}`);
            continue;
          }
        }
...
async function main() {
  ...
  log('INFO', `📦 Phase1后持仓: ${Object.keys(positions).length}个 keys=${Object.keys(positions).map(k=>positions[k]?.symbol||k.slice(0,8)).join(',')}`);
  ...
  // 持仓管理
  managePositions().catch(e => log('ERROR', `持仓管理崩溃: ${e.message}`));
  ...
  setInterval(async () => {
    try {
      ...
      const oldSol = new Set(solWalletSet);
      const oldBsc = new Set(bscWalletSet);
      const oldBase = new Set(baseWalletSet);
      // 先清空旧set，重建只包含猎手+哨兵的
      ...
      // 只在钱包列表真正变化时才重连WS（避免每4小时丢30秒信号）
      const setChanged = (a, b) => a.size !== b.size || [...a].some(x => !b.has(x));
      const solChanged = setChanged(solWalletSet, oldSol);
      const bscChanged = setChanged(bscWalletSet, oldBsc);
      const baseChanged = setChanged(baseWalletSet, oldBase);
      if (solChanged || bscChanged || baseChanged) {
        log('INFO', `  🔌 钱包列表变化→重连WS(SOL:${solChanged} BSC:${bscChanged} Base:${baseChanged})`);
        if (solChanged && CONFIG.enabledChains.includes('solana')) setupOfficialSolWs();
        if (bscChanged && CONFIG.enabledChains.includes('bsc')) setupEvmWebSocket('bsc');
        if (baseChanged && CONFIG.enabledChains.includes('base')) setupEvmWebSocket('base');
      } else {
        log('INFO', `  ✅ 钱包列表无变化，WS保持连接`);
      }
      log('INFO', `  排名更新: ${rankedWallets.length}个钱包 (库${Object.keys(walletDb).length}个)`);
      
      // 定时清理过期的pendingSignals（防内存泄漏）
      const now = Date.now();
      let cleaned = 0;
      for (const [token, sigs] of Object.entries(pendingSignals)) {
        const valid = sigs.filter(s => now - s.timestamp < CONFIG.confirmWindowMs);
        if (valid.length === 0) { delete pendingSignals[token]; cleaned++; }
        else if (valid.length < sigs.length) { pendingSignals[token] = valid; }
      }
      if (cleaned > 0) { savePendingSignals(); log('INFO', `🧹 清理过期pendingSignals: ${cleaned}个`); }
    } catch(e) {
  ...
  setTimeout(async () => {
    for (const [token, sigs] of Object.entries(pendingSignals)) {
      ...
      if (!CONFIG.enabledChains.includes(chain)) continue;
